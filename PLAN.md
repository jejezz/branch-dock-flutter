...
- `P2` 산출물 업로드 / 다운로드 / 삭제 (`gh release upload|download|delete-asset`). 같은 이름을
  덮어쓰면(`--clobber`) 이미 받아 간 사람과 내용이 달라지므로 한 번 더 확인한다.
...
- `P2` 리뷰 승인 / 변경 요청 / 코멘트 (`gh pr review`, `gh pr comment`)와 최근 리뷰·코멘트 보기.
  내가 만든 PR에는 GitHub가 승인·변경 요청을 막으므로 코멘트만 보인다.
